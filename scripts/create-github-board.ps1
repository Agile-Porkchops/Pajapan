...
param(
    ...
    # Skip the board entirely and work from the Issues tab alone.
    [switch]$SkipProject
)
...
if ($SkipProject) {
    Write-Host "== project == (skipped)" -ForegroundColor DarkGray
...
if ($proj) {
    ...
    # Surfaces the board on the repository's Projects tab. Safe to re-run.
    ...
    # ------------------------------------------------------- board fields --
    # Milestone and Labels are built-in fields, so risk / model / area are already
    # covered by the labels set above. Only add what labels can't express.
    $n = $proj.number
    $have = (gh project field-list $n --owner $Owner --format json | ConvertFrom-Json).fields.name

    if ($have -notcontains 'Priority') {
        gh project field-create $n --owner $Owner --name 'Priority' --data-type SINGLE_SELECT `
            --single-select-options 'P0 · Blocker,P1 · Critical path,P2 · Needed for launch,P3 · Polish' | Out-Null
        Write-Host '  + field Priority' -ForegroundColor Green
    }
    if ($have -notcontains 'Size') {
        gh project field-create $n --owner $Owner --name 'Size' --data-type SINGLE_SELECT `
            --single-select-options 'S · half day,M · 1 day,L · 2+ days' | Out-Null
        Write-Host '  + field Size' -ForegroundColor Green
    }

    # Build the query into a variable first — passing a here-string straight to
    # `-f query=@"..."@` gets mangled by PowerShell's parameter parser.
    $q = @"
query {
  organization(login: "$Owner") {
    projectV2(number: $n) {
      ...
      size:     field(name: "Size")     { ... on ProjectV2SingleSelectField { id options { id name } } }
    }
  }
}
"@
    $meta = (gh api graphql -f "query=$q" | ConvertFrom-Json).data.organization.projectV2
    if (-not $meta) { throw 'Could not read project fields — is the project number right?' }

    # Default Status is Todo / In Progress / Done. Add the two states this workflow
    # actually has: a PR awaiting review, and a task stuck on a dependency.
    if ($meta.status.options.name -notcontains 'Blocked') {
        $m = @"
mutation { updateProjectV2Field(input: { fieldId: "$($meta.status.id)" singleSelectOptions: [
  {name:"Todo",        color:GRAY,   description:"Not started"}
  {name:"In progress", color:YELLOW, description:"Being worked in a session"}
  {name:"In review",   color:PURPLE, description:"PR open, awaiting review"}
  {name:"Blocked",     color:RED,    description:"Waiting on a dependency or a decision"}
  {name:"Done",        color:GREEN,  description:"Merged and Done-when verified"} ] }) { clientMutationId } }
"@
        gh api graphql -f "query=$m" | Out-Null
        # Option ids are regenerated by the update, so re-read them.
        $meta = (gh api graphql -f "query=$q" | ConvertFrom-Json).data.organization.projectV2
        Write-Host '  ~ Status options customized' -ForegroundColor Green
    }

    # ------------------------------------------------- per-item field values --
    # Priority is derived, not hand-maintained: it falls out of the dependency
    # order in tasks/00-INDEX.md. Size is a default of M with two exception lists.
    $large = 'M1-01 M1-06 M2-06 M2-09 M4-04 M4-05 M6-02 M7-01 M7-07' -split ' '
    $small = 'M0-01 M1-02 M1-07 M2-03 M3-02 M4-08 M5-01 M5-06 M6-01' -split ' '
    $optId = { param($f, $name) ($f.options | Where-Object name -eq $name).id }

    $items = gh project item-list $n --owner $Owner --format json --limit 200 |
             ConvertFrom-Json | Select-Object -ExpandProperty items
    foreach ($it in $items) {
        $id = ($it.content.title -split ' ')[0]
        $p = switch -Regex ($id) {
            '^M0-|^M1-01$'                  { 'P0 · Blocker';            break }
            '^M[1-4]-|^M7-01$'              { 'P1 · Critical path';      break }
            '^M[56]-|^M7-0[78]$'            { 'P2 · Needed for launch';  break }
            default                         { 'P3 · Polish' }
        }
        $s = if ($large -contains $id) { 'L · 2+ days' }
             elseif ($small -contains $id) { 'S · half day' } else { 'M · 1 day' }

        gh project item-edit --id $it.id --project-id $meta.id --field-id $meta.priority.id `
            --single-select-option-id (& $optId $meta.priority $p) | Out-Null
        gh project item-edit --id $it.id --project-id $meta.id --field-id $meta.size.id `
            --single-select-option-id (& $optId $meta.size $s) | Out-Null
        if (-not $it.status) {
            gh project item-edit --id $it.id --project-id $meta.id --field-id $meta.status.id `
                --single-select-option-id (& $optId $meta.status 'Todo') | Out-Null
        }
    }
    Write-Host "  set Priority/Size/Status on $($items.Count) items" -ForegroundColor Green
}
